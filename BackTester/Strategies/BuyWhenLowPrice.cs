...
{
	public class BuyWhenLowPrice : IStrategy
	{
		...
		private List<PriceRecord> GetMinimalCoveredPrices(List<PriceRecord> datePrices, List<PriceRecord> prices)
		{
			...
			foreach (var sym in symbolsTrading)
			{
				...
				#region
				//foreach (var lowPrice in symbolPrevLowPrices)
				//{
				//	var index = symbolPrevLowPrices.IndexOf(lowPrice);

				//	int range = 1;
				//	do
				//	{
				//		PriceRecord leftPrice, rightPrice;
				//		if (range > index || range > symbolPrevLowPrices.Count - index - 1)
				//			break;

				//		leftPrice = symbolPrevLowPrices[index - range];
				//		rightPrice = symbolPrevLowPrices[index + range];

				//		if (leftPrice.Low < lowPrice.Low || rightPrice.Low < lowPrice.Low)
				//			break;

				//		range++;

				//	} while (range < symbolPrevLowPrices.Count / 2);
				//}
				#endregion
			}
